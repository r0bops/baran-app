...
import {
  ...
  base64urlEncode,
  base64urlDecode,
} from '../../baran-core-ts/dist/index.js';
...
const DATA = process.env.BARAN_DATA || join(__dirname, '..', 'data', 'records.jsonl');
// Pinned bridge identity (Big Peer). In production this seed lives in a KMS.
const BRIDGE_SEED = process.env.BARAN_BRIDGE_SEED || 'b0'.repeat(32);
const ADMINS = (process.env.BARAN_ADMINS || '').split(',').filter(Boolean);
...
server.listen(PORT, () => {
  console.log(`\n🟢 Baran API server (real) at http://localhost:${PORT}`);
