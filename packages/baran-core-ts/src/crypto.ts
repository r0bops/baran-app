// baran-core-ts: crypto (Ed25519 sign/verify, device_id, content_hash)
...
export function keyFromSeed(seedHex: string): BaranIdentity {
...
export function publicKeyFromRaw(rawPub: Uint8Array): ReturnType<typeof createPublicKey> {
...
export function sign(recordNoSig: Record<string, unknown>, signer: BaranIdentity): SignResult {
...
export function verify(record: Record<string, unknown>, pubRaw: Uint8Array): boolean {
  const pubKey = publicKeyFromRaw(pubRaw);
  const { sig, content_hash, ...noSig } = record;
  const msg = Buffer.from(canon(noSig), 'utf8');
  const sigBytes = Buffer.from(base64urlDecode(record.sig as string));
  return nodeVerify(null, msg, pubKey, sigBytes);
}

export function signPayload(payload: Record<string, unknown>, signer: BaranIdentity): string {
