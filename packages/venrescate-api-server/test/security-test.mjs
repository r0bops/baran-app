...
import { keyFromSeed, sign as signRecord, signPayload, canon, base64urlDecode } from '../../venrescate-core-ts/dist/index.js';
...
const dataDir = mkdtempSync(join(tmpdir(), 'venrescate-sec-'));
...
const srv = spawn('node', [SERVER], {
  env: { ...process.env, PORT: String(PORT), VENRESCATE_DATA: join(dataDir, 'r.jsonl'), VENRESCATE_AUTH_SECRET: SECRET },
  stdio: 'ignore',
});
...
try {
  ...
  console.log('VenRescate adversarial security suite (server)\n');
