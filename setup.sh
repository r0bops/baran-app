...
( cd packages/baran-core-ts && npm install && npm run build )
...
Done. To run:
  ...
  Backend (real API, :3001):   node packages/baran-api-server/src/server.mjs
  Backend (Phase-1 stub):      node packages/baran-api-stub/src/server.mjs
  Coordinator console (:5173): ( cd apps/coordinator-web && npm run dev )

Tests:
  TS vectors:        ( cd packages/baran-core-ts && npm test )
  Core adversarial:  ( cd packages/baran-core-ts && npm run test:security )
  API integration:   node test/integration-test.mjs
  Backend suites:    ( cd packages/baran-api-server && npm test && npm run test:security )
  Kotlin core:       ./gradlew :baran-core:test
